  ...
  definition(t) {
    ...
    t.field('startTrial', {
      ...
      async resolve(source, { id }, ctx) {
        ...
        const price = process.env.STRIPE_PRODUCT_ID_TRIAL
        const priceOfProduct = await stripe.prices.retrieve(price!)
        ...
        if (builder?.stripeAccountId && account?.details_submitted) {
          ...
          if (!lastPrice) {
            ...
            const product = products.data.find(
              (product) =>
                product.name ==
                (builder.companyName! || 'Instant App') + '_trial',
            )
            console.log('🚀 ~ file: plan.ts:77 ~ resolve ~ product:', product)
            try {
              lastPrice = (
        ...
        try {
          ...
          console.log('🚀 ~ file: plan.ts:395 ~ resolve ~ error:', error)
        }
        const metadata: any = {
          builderName: ctx.builderDomain.split('.')[0],
          userId: user?.id.toString(),
...
export const createSubscription = async (id: number) => {
  ...
  if (builder?.stripeAccountId && account?.details_submitted) {
    ...
    if (!lastPrice) {
      ...
      console.log('🚀 ~ file: plan.ts:74 ~ resolve ~ products:', products)
      const product = products.data.find(
        (product) => product.name == (builder.companyName! || 'Instant App'),
      )
      console.log('🚀 ~ file: plan.ts:77 ~ resolve ~ product:', product)
      try {
        lastPrice = (
          await stripe.prices.create(
            {
              ...
              recurring: {
                ...
                // aggregate_usage: priceOfProduct.recurring?.aggregate_usage!,
                trial_period_days:
                  priceOfProduct.recurring?.trial_period_days || undefined,
                usage_type: priceOfProduct.recurring?.usage_type!,
              },
              product_data: !product?.id
                ? {
        ...
        console.log('🚀 ~ file: plan.ts:116 ~ resolve ~ lastPrice:', lastPrice)
      } catch (error) {
        console.log('🚀 ~ file: plan.ts:120 ~ resolve ~ error:', error)
        throw new Error('Something went wrong')
      }
  ...
  const metadata: any = {
    builderName: builder?.domain?.split('.')[0],
    userId: user?.id.toString(),
    appId: app?.id.toString(),
    utm: user?.registerFrom,
    utmPlan: user?.utmPlan,
    trial: trialNumber,
    apollotrial:
      user?.registerFrom == 'apollosale' && trialNumber ? 'yes' : undefined,
  }
  let isCustomerExist: any = false
  console.log(
    '🚀 ~ file: plan.ts:88 ~ resolve ~ isCustomerExist:',
